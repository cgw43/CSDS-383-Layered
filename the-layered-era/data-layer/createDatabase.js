...
function createDB() {
    ...
    // Create the database
    const createDatabaseQuery = `CREATE DATABASE "event_manager";`;

    pgClient.query(createDatabaseQuery, (err, result) => {
        if (err) {
            ...
            const newConnectionString = "postgres://postgres:password@localhost:5432/event_manager";
            const newPgClient = new pg.Client(newConnectionString);
            newPgClient.connect();

            // Define an array of queries to create tables and functions
            const createTablesAndFunctionsQueries = [
                `CREATE TABLE events (
            ...
            // Use the multi method to execute multiple queries
            newPgClient.query(createTablesAndFunctionsQueries.join(' '), (err, result) => {
                if (err) {
                    console.error('Error creating tables and functions:', err);
                } else {
